...
**Działanie naprawcze:** Dodano komentarz ostrzegawczy w kodzie (PR: security/2026-04-14-review). **Klucz musi być ręcznie zrotowany** w panelu JSONBin.io → Settings → API Keys.
...
**Naprawione (PR: security/2026-04-14-review):** Dodano `console.warn` przy starcie serwera gdy env var brakuje.
...
**Naprawione (PR: security/2026-04-14-review):** Zaktualizowano `bcrypt ^5.1.1` → `^6.0.0` w `package.json`.
...
**Działanie (PR: security/2026-04-14-review):** Dodano `console.warn` przy inicjalizacji. Hasła nie zmieniono (logika biznesowa).

---

#### [JAKOŚĆ — MARTWY KOD] Nieużywana zmienna `trafficCache` w `seo/router.js:93`

```js
const trafficCache = cache.get('ga4-traffic'); // nigdy nie używana dalej
```

**Naprawione (PR: quality/2026-04-14-review):** Zmienna usunięta. Dodano komentarz TODO o brakującej logice `organicSessions`.

#### [JAKOŚĆ — BUG] Błędna kolejność sprawdzeń w `PUT /api/admin/users/:username` (`seo/router.js`)

Przed poprawką: `changePassword()` wywoływane przed sprawdzeniem czy użytkownik istnieje → przy nieistniejącym użytkowniku błąd 400 zamiast 404.

**Naprawione (PR: quality/2026-04-14-review):** Sprawdzenie istnienia użytkownika przeniesione na początek handlera.
...
**Działanie (PR: quality/2026-04-14-review):** Dodano komentarz TODO w kodzie. Nie naprawiono — wymaga decyzji projektowej o źródle danych `organicSessions`.
...
| `seo/router.js` | Usunięcie martwej zmiennej `trafficCache` + komentarz TODO | quality/2026-04-14-review |
| `seo/router.js` | Kolejność sprawdzeń w PUT users/:username (404 przed zmianą hasła) | quality/2026-04-14-review |
